...
async function transferStudent(student, fromSide) {
    ...
    let targetAcademicId = null;

    if (fromSide === "left") {
        
        targetAcademicId = academicId2;
    } else {
        
        targetAcademicId = academicId1;
    }
...
searchInput.addEventListener("input", function () {
    ...
    const filteredLeft = leftStudents.filter(filterLogic);
    const filteredRight = rightStudents.filter(filterLogic);

    printStudents(filteredLeft, "left");
    printStudents(filteredRight, "right");
});
